...
Node WebServer
  {
    ...
    {
        TestScript = {
    		Test-Path -Path "C:\aadsc-app.zip"
        }
        SetScript ={
			$source  = "https://github.com/bhummerstone/iac-hackathon-dsc/raw/master/aadsc-app.zip"
			$dest    = "C:\aadsc-app.zip"
			Invoke-WebRequest $source -OutFile $dest
        }
    ...
    {
        TestScript = {
    		Test-Path -Path "HKLM:\SOFTWARE\DSC-Software\AADSCWebDeployPkgInstalled"
        }
        SetScript ={
			...
			& $msDeployPath "-verb:sync", "-source:package=C:\aadsc-app.zip", "-dest:auto,ComputerName=""localhost""", "-setParam:name=""$appName"",value=""$siteName"""
		    
			if ($LASTEXITCODE -eq 0) 
			{
				New-Item -Path "HKLM:\SOFTWARE\DSC-Software\AADSCWebDeployPkgInstalled" -Force
			}
        }
